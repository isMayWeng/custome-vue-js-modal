            ...
            <div :class="backgroundClickClass"
                ...
                <transition :name="transition">
                    <div v-if="visibility.modal"
                         ...
                         @mousedown.stop
                         @touchstart.stop>
                        <div class="v--modal-header" v-if="showHeader">
                            <div class="title" v-html="title"></div>
                            <button class="btn-close" type="button"
                                    @click.stop="toggle(false)"><i class="icon-close"></i></button>
                        </div>
                        <slot/>
...
<script>
    ...
    export default {
        ...
        beforeMount() {
            Modal.event.$on('toggle', this.toggleFunc)
            Modal.event.$on('close', this.closeFunc)
         ...
         */
        beforeDestroy() {
            Modal.event.$off('toggle', this.toggleFunc)
            Modal.event.$off('close', this.closeFunc)
        ...
        methods: {
            ...
            closeFunc(name, state, params) {
                this.isCloseNotCancel = true;
                this.toggleFunc(name, state, params)
            },
...
<style>
    ...
    .v--modal .v--modal-header {
        font-size: 16px;
        background: #FAFAFA;
        box-shadow: 0 1px 0 0 rgba(207, 207, 207, 0.50);
        font-weight: 600;
    ...
    .v--modal .v--modal-header .title {
        display: inline-block;
    }

    .v--modal .v--modal-header .btn-close {
        display: inline-block;
        float: right;
